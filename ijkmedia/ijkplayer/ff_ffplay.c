...
int meta_queue_init(MetaDataQueue *queue, int size)
{
    ...
    queue->data = (MetaData **)malloc(size * sizeof(MetaData *));
    queue->size = size;
    queue->last = -1;
    return 0;
}

void meta_queue_destroy(MetaDataQueue *queue)
{
    SDL_DestroyMutex(queue->mutex);
    free(queue->data);
}

MetaData *meta_queue_get(MetaDataQueue *queue)
{
    ...
    MetaData *data = queue->data[0];
    SDL_UnlockMutex(queue->mutex);
    return data;
}

MetaData *meta_queue_pop(MetaDataQueue *queue)
{
    SDL_LockMutex(queue->mutex);
    if (queue->last < 0) {
        SDL_UnlockMutex(queue->mutex);
...
void meta_queue_put(MetaDataQueue *queue, MetaData *data)
{
    SDL_LockMutex(queue->mutex);
...
int SEI_user_data(AVPacket *pkt, enum AVCodecID codecId)
{
    if (codecId == AV_CODEC_ID_H264 || codecId == AV_CODEC_ID_MJPEG) {
        return pkt->data[0] == 0x0 && pkt->data[1] == 0x0 && pkt->data[2] == 0x0 && 
            pkt->data[3] == 0x1 && pkt->data[4] == 0x6 && pkt->data[5] == 0x5;
    } else if (codecId == AV_CODEC_ID_HEVC) {
        int a = pkt->data[0] == 0x0 && pkt->data[1] == 0x0 && pkt->data[2] == 0x0 && 
            pkt->data[3] == 0x1 && (pkt->data[4] == 0x4E || pkt->data[4] == 0x50) && pkt->data[5] == 0x1 && pkt->data[6] == 0x5;
        int b = pkt->data[0] == 0x0 && pkt->data[1] == 0x0 && pkt->data[2] == 0x1 &&
            (pkt->data[3] == 0x4E || pkt->data[3] == 0x50) && pkt->data[4] == 0x1 && pkt->data[5] == 0x5;
        return a || b;
    } else {
        return 0;
    }
...
unsigned char *get_SEI_payload(AVPacket *pkt, enum AVCodecID codecId) {
    unsigned char *data;
    if (codecId == AV_CODEC_ID_H264 || codecId == AV_CODEC_ID_MJPEG) {
        data = &pkt->data[6];
    } else if (codecId == AV_CODEC_ID_HEVC) {
    ...
    unsigned char *meta = (unsigned char *)malloc(payload_size + 1);
    memcpy(meta, data, payload_size);
    meta[payload_size] = 0;

    int meta_size = 23 + payload_size_size + payload_size;
    int remain_size = pkt->size - meta_size;
    if (remain_size > 0) {
        memmove(pkt->data, &pkt->data[meta_size], remain_size);
        av_shrink_packet(pkt, remain_size);
    }
...
long ffp_get_frame_l(FFPlayer *ffp, uint8_t **data, int *width, int *height, unsigned char **meta)
{
    ...
    if (is == NULL || is->rgba_data == NULL) {
        return -1;
    }
    
    SDL_LockMutex(is->frame_mutex);
    *width = is->frame_width;
    *height = is->frame_height;
    *data = (uint8_t *)malloc(*width * *height * 4);
    memcpy(*data, is->rgba_data, *width * *height * 4);
    *meta = is->meta ? is->meta->meta : NULL;
    if (is->meta) {
        int len = strlen(is->meta->meta);
        *meta = (unsigned char *)malloc(len + 1);
        memcpy(*meta, is->meta->meta, len + 1);
    }
...
static int put_to_frame_buffer(FFPlayer *ffp, Frame* frame)
{
    VideoState *is = ffp->is;
    ...
    if (w != is->frame_width || h != is->frame_height) {
        is->frame_width = w;
        is->frame_height = h;
        video_changed = 1;
    }
    ...
    if (!is->rgba_data || video_changed) {
        ...
        is->rgba_data = (uint8_t *)malloc(is->frame_width * is->frame_height * 4);
    }

    uint8_t *src[3]= {frame->bmp->pixels[0], frame->bmp->pixels[1], frame->bmp->pixels[2]};
    int src_stride[3]={frame->bmp->pitches[0], frame->bmp->pitches[1], frame->bmp->pitches[2]};
    uint8_t *dst[3]= {is->rgba_data, NULL, NULL};
    int stride[3]={w*4, 0, 0};

    if (!is->yuv_to_rgba_ctx || video_changed) {
        if (is->yuv_to_rgba_ctx) {
            sws_freeContext(is->yuv_to_rgba_ctx);
        }
        enum AVPixelFormat src_fmt = getPixelFormat(frame->bmp);
        is->yuv_to_rgba_ctx = sws_getContext(w, h, src_fmt, w, h, AV_PIX_FMT_0BGR32, SWS_POINT, NULL, NULL, NULL);
    }
    sws_scale(is->yuv_to_rgba_ctx, src, src_stride, 0, h, dst, stride);
    SDL_UnlockMutex(is->frame_mutex);
...
static void video_image_display2(FFPlayer *ffp)
{
    ...
    if (ffp->frame_dropped) {
        if (ffp->stat.vfps > ffp->avg_fps * 0.85f) {
            ffp->frame_dropped = false;
            ffp_notify_msg1(ffp, FFP_MSG_FRAME_NOT_DROPPED);
        }
    } else {
        if (ffp->avg_fps > 0.0f && ffp->stat.vfps > 0.0f) {
            if (ffp->stat.vfps < ffp->avg_fps * 0.6f) {
                ffp->frame_dropped = true;
                ffp_notify_msg1(ffp, FFP_MSG_FRAME_DROPPED);
            }
        }
    }
    ...
    if (ffp->low_delay) {
        int64_t vcache = ffp->stat.video_cache.duration;
        int64_t acache = is->audio_st ? ffp->stat.audio_cache.duration : 0;
        int64_t cache = FFMAX(vcache, acache);
        if (cache >= ffp->low_delay_start_threshold && !ffp->high_speed_playback) {
            ffp_set_playback_rate(ffp, 2.0f);
            ffp->high_speed_playback = 1;
        } else if (cache <= ffp->low_delay_stop_threshold && ffp->high_speed_playback) {
            ffp_set_playback_rate(ffp, 1.0f);
            ffp->high_speed_playback = 0;
        }
    } else {
        if (ffp->high_speed_playback) {
            ffp_set_playback_rate(ffp, 1.0f);
            ffp->high_speed_playback = 0;
        }
    }
    ...
    if (vp->bmp) {
        ...
        if (ffp->enable_get_frame) {
            put_to_frame_buffer(ffp, vp);
        } else {
            if (!is->frame_width) {
                is->frame_width = vp->bmp->w;
            }
            if (!is->frame_height) {
                is->frame_height = vp->bmp->h;
            }
        }
...
static void stream_close(FFPlayer *ffp)
{
    ...
    MetaData *data;
    while((data = meta_queue_pop(&is->metaq)) != NULL) {
        free(data->meta);
        free(data);
    }
    meta_queue_destroy(&is->metaq);
...
retry:
        if (frame_queue_nb_remaining(&is->pictq) == 0) {
            ...
            if (!isnan(vp->pts)) {
                ...
                ffp->stat.video_frame_timestamp = (int64_t)(vp->pts * 1000LL);
            }
#ifdef AVTECH
            is->avtech_timestamp = vp->avtech_timestamp;
#endif
...
static int queue_picture(FFPlayer *ffp, AVFrame *src_frame, double pts, double duration, int64_t pos, int serial)
{
    ...
    if (!ffp->is->audio_st) {
        src_frame->pts /= ffp->pf_playback_rate;
        pts /= ffp->pf_playback_rate;
    }

    if (ffp->enable_accurate_seek && is->video_accurate_seek_req && !is->seeking) {
        if (!isnan(pts)) {
            ...
            if ((pts * 1000 * 1000 < is->seek_pos) || deviation > MAX_DEVIATION) {
                ...
                while (is->audio_accurate_seek_req && !is->abort_request && !is->audclk.paused) {
                ...
                if (video_seek_pos == is->seek_pos) {
                    ...
                    if (video_seek_pos == is->seek_pos && is->audio_accurate_seek_req && !is->abort_request && !is->audclk.paused) {
        ...
        if (video_accurate_seek_fail) {
            ...
            is->video_accurate_seek_req = 0;
            SDL_CondSignal(is->audio_accurate_seek_cond);
            if (is->audio_accurate_seek_req && !is->abort_request && !is->audclk.paused) {
                SDL_CondWaitTimeout(is->video_accurate_seek_cond, is->accurate_seek_mutex, ffp->accurate_seek_timeout);
            } else {
                if (!isnan(pts)) {
                    ffp_notify_msg2(ffp, FFP_MSG_ACCURATE_SEEK_COMPLETE, (int)(pts * 1000));
...
#ifdef AVTECH
        vp->avtech_timestamp = src_frame->avtech_timestamp;
#endif
#ifdef FFP_MERGE
...
#define INSERT_FILT(name, arg) do {                                          \
    ...
    if (fabsf(ffp->pf_playback_rate) > 0.00001 &&
        fabsf(ffp->pf_playback_rate - 1.0f) > 0.00001 && !ffp->is->audio_st) {
        char setpts_buf[256];
...
static int audio_thread(void *arg)
{
    ...
    do {
        ...
        if (got_frame) {
                ...
                if (ffp->enable_accurate_seek && is->audio_accurate_seek_req && !is->seeking) {
...
static void sdl_audio_callback(void *opaque, Uint8 *stream, int len)
{
    ...
    while (len > 0) {
        ...
        if (len1 > len)
            len1 = len;
        if (!is->muted && is->audio_buf && is->audio_volume == SDL_MIX_MAXVOLUME && ffp->pf_playback_volume > 0.f)
...
static int audio_open(FFPlayer *opaque, int64_t wanted_channel_layout, int wanted_nb_channels, int wanted_sample_rate, int wanted_bits_per_sample, struct AudioParams *audio_hw_params)
{
    ...
    switch(wanted_bits_per_sample) {
        case 8: audio_sample_format = AUDIO_U8; break;
        case 16: audio_sample_format = AUDIO_S16SYS; break;
        case 32: audio_sample_format = AUDIO_S32SYS; break;
        default: audio_sample_format = AUDIO_S16SYS; break;
    }

    wanted_spec.format = audio_sample_format;
...
static int stream_component_open(FFPlayer *ffp, int stream_index)
{
...
#endif
        int bits_per_sample = ic->streams[stream_index]->codecpar->bits_per_coded_sample;

        /* prepare audio output */
        if ((ret = audio_open(ffp, channel_layout, nb_channels, sample_rate, bits_per_sample, &is->audio_tgt)) < 0)
            goto fail;
    ...
    case AVMEDIA_TYPE_VIDEO:
#ifdef AVTECH
        avctx->disable_multithread_delaying = ffp->disable_multithread_delaying;
#endif
...
static inline bool isVideoRecordEnable(FFPlayer *ffp) {
    return ffp->video_record_path && ffp->video_record_path[0];
}

static int find_video_stream(AVFormatContext *ic) {
    if (ic == NULL) {
        return -1;
    }
...
static int find_audio_stream(AVFormatContext *ic) {
    if (ic == NULL) {
        return -1;
    }
...
static void fix_stream_codec_parameters(AVStream *stream) {
    uint32_t h264_tag = MKTAG('a', 'v', 'c', '1');
    uint32_t h265_tag = MKTAG('h', 'v', 'c', '1');
    if (stream->codecpar->codec_id == AV_CODEC_ID_H264 && stream->codecpar->codec_tag != h264_tag) {
        stream->codecpar->codec_tag = h264_tag;
    } else if (stream->codecpar->codec_id == AV_CODEC_ID_HEVC && stream->codecpar->codec_tag != h265_tag) {
        //
        // <HACK>: ios only support hvc1 tag
        //
        stream->codecpar->codec_tag = h265_tag;
    }
}

static AVFormatContext *start_video_record(FFPlayer *ffp)
{
    ...
    for (int i = 0; i < ic->nb_streams; i++) {
        if (ic->streams[i]->codecpar->codec_type == AVMEDIA_TYPE_VIDEO) {
            w = ic->streams[i]->codecpar->width;
            h = ic->streams[i]->codecpar->height;
        }
    }
    ...
    avformat_alloc_output_context2(&oc, NULL, NULL, ffp->video_record_path);
    int video_index = find_video_stream(ic);
    if (video_index >= 0 ) {
        AVStream *in_stream = ic->streams[video_index];
        AVCodec *codec = avcodec_find_decoder(in_stream->codecpar->codec_id);
        AVStream *out_stream = avformat_new_stream(oc, codec);

        AVCodecContext *codec_ctx = avcodec_alloc_context3(codec);
        in_stream->codecpar->width = w;
        in_stream->codecpar->height = h;
        ...
        //
        // <INFO> mp4 muxer would adjust timebase if timebase.den < 10000, use video_track_timescale to fix avapi recorded video timebase is wrong issue
        //
        av_dict_set_int(&opt, "video_track_timescale", out_stream->time_base.den, 0);

        fix_stream_codec_parameters(out_stream);
    }

    int audio_index = find_audio_stream(ic);
    if (audio_index >= 0 ) {
        AVStream *in_stream = ic->streams[audio_index];
        ...
        codec_ctx->sample_rate    = is->audio_st->codecpar->sample_rate;
    ...
    int ret = avio_open(&oc->pb, ffp->video_record_path, AVIO_FLAG_WRITE);
    if (ret < 0) {
        av_log(NULL, AV_LOG_ERROR, "Error occurred when opening output file %s\n", ffp->video_record_path);
...
int64_t encode_audio_frame(AVFormatContext *output_context, AVCodecContext *output_codec_context, AVFrame *frame, int drop_frame)
{
    int audio_index = find_audio_stream(output_context);
    AVPacket pkt;
    av_init_packet(&pkt);
    avcodec_send_frame(output_codec_context, frame);
    int ret = avcodec_receive_packet(output_codec_context, &pkt);
...
static int stop_video_record(AVFormatContext *oc, TranscodeContext *ctx) {
    if (!oc) {
        return 0;
    }

    if (ctx->output_codec_context) {
        while (encode_audio_frame(oc, ctx->output_codec_context, NULL, 1) != AV_NOPTS_VALUE) {
        }
    }
    ...
    if (ctx->resample_context) {
        swr_free(&ctx->resample_context);
        ctx->resample_context = NULL;
    }
    if (ctx->output_codec_context) {
        avcodec_free_context(&ctx->output_codec_context);
        ctx->output_codec_context = NULL;
    }
    if (ctx->input_codec_context) {
        avcodec_free_context(&ctx->input_codec_context);
        ctx->input_codec_context = NULL;
    }
    ctx->pts = 0;
...
AVCodecContext *open_audio_input(AVStream *stream)
{
...
AVCodecContext *open_audio_output(AVStream *stream)
{
    ...
    context->channels       = stream->codecpar->channels;
...
SwrContext *init_resampler(AVCodecContext *input_codec_context, AVCodecContext *output_codec_context)
{
...
void init_audio_transcode_context(TranscodeContext *context, AVStream *in_audio_st, AVStream *out_audio_st)
{
    if (!context->input_codec_context) {
        context->input_codec_context = open_audio_input(in_audio_st);
    }

    if (!context->output_codec_context) {
        context->output_codec_context = open_audio_output(in_audio_st);
    }

    if (!context->resample_context) {
        context->resample_context = init_resampler(context->input_codec_context, context->output_codec_context);
    }

    if (!context->fifo) {
        context->fifo = av_audio_fifo_alloc(context->output_codec_context->sample_fmt, context->output_codec_context->channels, 1);
    }

    context->pts = 0;

    out_audio_st->codecpar->extradata = av_malloc(context->output_codec_context->extradata_size + FF_INPUT_BUFFER_PADDING_SIZE);
    out_audio_st->codecpar->extradata_size = context->output_codec_context->extradata_size;
    memcpy(out_audio_st->codecpar->extradata, context->output_codec_context->extradata, context->output_codec_context->extradata_size);
}

void decode_and_convert(TranscodeContext *ctx, AVPacket *pkt)
{
    AVFrame *frame = av_frame_alloc();
    avcodec_send_packet(ctx->input_codec_context, pkt);
    int ret = avcodec_receive_frame(ctx->input_codec_context, frame);
    ...
    uint8_t **converted_input_samples = calloc(ctx->output_codec_context->channels, sizeof(*converted_input_samples));
    av_samples_alloc(converted_input_samples, NULL,
                    ctx->output_codec_context->channels,
                    nb_samples,
                    ctx->output_codec_context->sample_fmt, 0);
    swr_convert(ctx->resample_context,
            converted_input_samples, nb_samples,
            (const uint8_t**)frame->extended_data, nb_samples);                             

    ret = av_audio_fifo_realloc(ctx->fifo, av_audio_fifo_size(ctx->fifo) + nb_samples);
    av_audio_fifo_write(ctx->fifo, (void **)converted_input_samples, nb_samples);
...
int64_t load_and_encode(TranscodeContext *ctx, AVFormatContext *output_context)
{
    AVFrame *frame;
    init_output_frame(&frame, ctx->output_codec_context, ctx->output_codec_context->frame_size);
    av_audio_fifo_read(ctx->fifo, (void **)frame->data, ctx->output_codec_context->frame_size);
    ...
    int64_t pts = encode_audio_frame(output_context, ctx->output_codec_context, frame, 0);
    av_frame_free(&frame);
    return pts;
} 
...
static int read_thread(void *arg)
{
    ...
    if (ffp->seek_at_start > 0) {
        ffp_seek_to_l(ffp, (uint64_t)(ffp->seek_at_start));
    }

    TranscodeContext ac;
    memset(&ac, 0, sizeof(TranscodeContext));
    int64_t prev_pts = -1;
    int64_t prev_video_pts = AV_NOPTS_VALUE;
    ...
    int64_t stop_recording_starttime_msec = 0;
    int64_t start_pts = AV_NOPTS_VALUE;
    int64_t total_size = 0;
    int64_t network_bitrate_start_time = -1;
    int64_t network_bitrate_cur_time;
    int64_t network_bitrate_total_size = 0;
...
//      of the seek_pos/seek_rel variables
...
#endif
        if (is->paused && ffp->hack_claire_control) {
            SDL_Delay(100);
            continue;
        }

        /* if the queue are full, no need to read more */
        if (ffp->infinite_buffer<1 && !is->seeking &&
#ifdef FFP_MERGE
        ...
        if (ret < 0) {
            ...
            if (ret == AVERROR_EOF || avio_feof(ic->pb)) {
                if (oc) {
                    int record_ret = stop_video_record(oc, &ac);
                    key_frame_found = false;
                    oc = NULL;
                    ffp->video_record_path = NULL;
                    ffp_notify_msg2(ffp, FFP_MSG_VIDEO_RECORD_COMPLETE, record_ret ? -1 : 0);
                }                
            }
        ...
        if (pkt->stream_index == is->video_stream) {
            ...
            int64_t msec = av_rescale_q(pkt->pts - start_pts, is->video_st->time_base, av_d2q(0.001, INT_MAX));
            if (msec >= BITRATE_MIN_INTERVAL_MSEC) {
                ffp->stat.bit_rate = (total_size * 8 * 1000) / msec;
            }
            if (msec >= BITRATE_MAX_INTERVAL_MSEC) {
                total_size = 0;
                start_pts = pkt->pts;
            }
            total_size += pkt->size;

            if (network_bitrate_start_time < 0) {
                network_bitrate_start_time =  av_gettime_relative() / 1000;
            }
            int64_t network_bitrate_msec = av_gettime_relative() / 1000 - network_bitrate_start_time;
            if (network_bitrate_msec >= BITRATE_MIN_INTERVAL_MSEC) {
                ffp->stat.network_bit_rate = (network_bitrate_total_size * 8 * 1000) / network_bitrate_msec;
            }            
            if (network_bitrate_msec >= BITRATE_MAX_INTERVAL_MSEC) {
                network_bitrate_total_size = 0;
                network_bitrate_start_time =  av_gettime_relative() / 1000;
            }
            network_bitrate_total_size += pkt->size;
        }
            
        if (is->video_st && SEI_user_data(pkt, is->video_st->codecpar->codec_id)) {
            unsigned char *meta = get_SEI_payload(pkt, is->video_st->codecpar->codec_id);
            ...
            meta_data->pts = pkt->pts;
            meta_queue_put(&is->metaq, meta_data);
            meta_data = NULL;
        }

        if (pkt->stream_index == is->video_stream && meta_data) {
            meta_data->pts = pkt->pts;
            meta_queue_put(&is->metaq, meta_data);
            meta_data = NULL;
        }
...
#ifdef AVTECH
        if (pkt->avtech_timestamp != 0 && is->avtech_start_timestamp == 0) {
            is->avtech_start_timestamp = pkt->avtech_timestamp - 1;
        }

        //
        // <INFO> update avtech_playback_status here to let app show black screen quickly
        //
        is->avtech_playback_status = pkt->avtech_playback_status;
#endif

        if (prev_pts != -1 && is->video_st) {
            uint64_t diff = pkt->pts - prev_pts;
            float fps =  1.0f / (diff * av_q2d(is->video_st->time_base));
            ffp->avg_fps = IJKMAX(ffp->avg_fps, fps);
        }
        prev_pts = pkt->pts;

        if (isVideoRecordEnable(ffp) && !oc) {
            oc = start_video_record(ffp);
            if (!oc) {
                ffp->video_record_path = NULL;
                ffp_notify_msg2(ffp, FFP_MSG_VIDEO_RECORD_COMPLETE, -1);
            } else {
                out_video_index = find_video_stream(oc);
                out_audio_index = find_audio_stream(oc);
                if (out_audio_index >= 0) {
                    init_audio_transcode_context(&ac, is->audio_st, oc->streams[out_audio_index]);
                }
            }
        }
        ...
        if (oc) {
            ...
            if (key_frame_found && (!video_record_stoping || audio_duration_msec < video_duration_msec)) {
                if (is->audio_st && pkt->stream_index == is->audio_st->index) {
                    decode_and_convert(&ac, pkt);
                    while (av_audio_fifo_size(ac.fifo) >= ac.output_codec_context->frame_size) {
                        audio_pts = load_and_encode(&ac, oc);
                        if (audio_start_pts == AV_NOPTS_VALUE && audio_pts != AV_NOPTS_VALUE) {
                            audio_start_pts = audio_pts;
                        }
                        audio_duration_msec = ((audio_pts - audio_start_pts) / is->auddec.avctx->sample_rate) * 1000;
                        uint64_t now = av_gettime_relative() / 1000;
                        if (video_record_stoping && (audio_duration_msec >= video_duration_msec || now - stop_recording_starttime_msec >= STOP_RECORDING_TIMEOUT_MSEC)) {
                            video_record_ready_to_stop = 1;
                            break;
                        }
                    }
            ...
            if (video_record_stoping && (audio_duration_msec == video_duration_msec || !is->audio_st || !is->video_st)) {
                video_record_ready_to_stop = 1;
            }
        }
        ...
        if (oc) {
            ...
            if (!isVideoRecordEnable(ffp)) {
                video_record_stoping = 1;
                if (!stop_recording_starttime_msec) {
                    stop_recording_starttime_msec = av_gettime_relative() / 1000;
                }
            }

            if (video_record_ready_to_stop) {
                video_record_stoping = 0;
                ...
                stop_recording_starttime_msec = 0;
                oc = NULL;
                int error = ret ? -1 : 0;
                if (video_record_error != 0) {
                    error = video_record_error;
                }
                video_record_error = 0;
                ffp_notify_msg2(ffp, FFP_MSG_VIDEO_RECORD_COMPLETE, error);
            }
        }
...
static VideoState *stream_open(FFPlayer *ffp, const char *filename, AVInputFormat *iformat)
{
    ...
    meta_queue_init(&is->metaq, META_QUEUE_SIZE);
...
#ifdef AVTECH
    int use_avtech_playback_timestamp = is->avtech_timestamp != 0;
    int64_t pos = 0;
    double pos_clock = get_master_clock(is);

    if (use_avtech_playback_timestamp) {
        if (ffp->video_seeking) {
            pos = fftime_to_milliseconds(is->seek_pos);
            pos = pos / 1000L;
            pos = (pos - is->avtech_start_timestamp) * 1000L;
        } else {
            pos = (is->avtech_timestamp - is->avtech_start_timestamp) * 1000L;
        }
    } else {
        if (isnan(pos_clock)) {
            pos = fftime_to_milliseconds(is->seek_pos);
...
uint32_t ffp_get_real_time_l(FFPlayer *ffp)
{
    ...
    double pos_clock = get_master_clock(is);
    return isnan(pos_clock) ? fftime_to_milliseconds(is->seek_pos) / 1000 : is->avtech_timestamp;
}
...
uint32_t ffp_get_avtech_playback_status_l(FFPlayer *ffp)
{
    ...
    return is->avtech_playback_status;
}
...
int64_t ffp_get_property_int64(FFPlayer *ffp, int id, int64_t default_value)
{
    switch (id) {
        ...
        case FFP_PROP_INT64_NETWORK_BIT_RATE:
            return ffp ? ffp->stat.network_bit_rate : default_value;
       ...
       case FFP_PROP_INT64_VIDEO_FRAME_TIMESTAMP:
            if (!ffp)
                return default_value;
            return ffp->stat.video_frame_timestamp;
        default:
            return default_value;
    }
}
